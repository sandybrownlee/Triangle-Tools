...
public class Parser {
	...
	Command parseSingleCommand() throws SyntaxError {
		...
		case Token.BEGIN:
			...
			acceptIt();
			if (currentToken.kind == Token.END) accept(Token.END);
			if (currentToken.kind == Token.RCURLY) accept(Token.RCURLY);
			break;
		...
		case Token.LOOP: {
			acceptIt();
			//accept(Token.BEGIN); - don't need this, it's included =D
			Command cAST = parseSingleCommand();
			accept(Token.WHILE);
			Expression eAST = parseExpression();
			accept(Token.DO);
			Command c2AST = parseSingleCommand();
			finish(commandPos);
			commandAST = new LoopMiddleCheckCommand(eAST,cAST, c2AST, commandPos);
		}
			break;
