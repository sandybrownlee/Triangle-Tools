...
public class ConstantFolder implements ActualParameterVisitor<Void, AbstractSyntaxTree>,
	...
	public AbstractSyntaxTree visitWhileCommand(WhileCommand ast, Void arg) {
		ast.C.visit(this);
		// anywhere an expression appears,
		AbstractSyntaxTree replacement = ast.E.visit(this);
		// if an updated expression is received
		if (replacement != null) {
	...
	public AbstractSyntaxTree foldBinaryExpression(AbstractSyntaxTree node1, AbstractSyntaxTree node2, Operator o) {
		...
		if ((node1 instanceof IntegerExpression) && (node2 instanceof IntegerExpression)) {
			...
			if (foldedValue instanceof Integer) {
				...
				// added
				String bool = foldedValue.toString().toLowerCase();
				Identifier i = new Identifier(bool, node1.getPosition());
				if (bool.equals("true")) { i.decl = StdEnvironment.trueDecl; }
				else { i.decl = StdEnvironment.falseDecl; }
				VnameExpression vne = new VnameExpression((new SimpleVname(i, node1.getPosition())), node1.getPosition());
